	...
	{		
		...
		case "Step_agree":
			...
			{ //бандюганы
				...
				pchar.GenQuest.OpenTheRopeExit = pchar.GenQuest.EncGirlF.locationId; //флаг дл€ открыти€ релоада
				string model[10];
				model[0] = "citiz_41";
				model[1] = "citiz_42";
				model[2] = "citiz_43";
				...
				while(i < 3)
				{
					...
					if (model[iMassive] != "")
					{
						...
						LAi_group_MoveCharacter(sld, LAI_GROUP_ENEMY);// лесник изменил группу чтобы €дом таино травить можно было
			...
			{ //и правда девка 
				...
				int iTemp = rand(1);
				sld.dialog.currentnode = "Underground"+ iTemp; //здесь рендом поведени€ девки: 0-помогите, 1-сама крута€// лесник - раскоментил itemp
		...
		case "Underground1": //крута€ мочалка
