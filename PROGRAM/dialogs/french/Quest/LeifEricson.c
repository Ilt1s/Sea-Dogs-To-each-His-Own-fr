#include "SD\TEXT\DIALOGS\Quest\LeifEricson.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            ...
            if (pchar.questTemp.Ascold == "Ascold_MummyFoundItems")
            {
                ...
                if  (CheckCharacterItem(pchar, "sculMa1") && CheckCharacterItem(pchar, "sculMa2") && CheckCharacterItem(pchar, "sculMa3") && CheckCharacterItem(pchar, "talisman6"))
                {
        ...
        case "Step_26":
            ...
            for (i=1; i<=6; i++)
            {
				sld = GetCharacter(NPC_GenerateCharacter("MySkel"+i, "Skel"+(rand(3)+1), "skeleton", "skeleton", 20, PIRATE, 0, true, "soldier"));	
				FantomMakeCoolFighter(sld, 20, 60, 60, "topor_02", "pistol5", "bullet", 100);  
        ...
        case "Step_29":
			...
			LocatorReloadEnterDisable("baster_housesp1", "reload2", false);
            //==> вертаем
