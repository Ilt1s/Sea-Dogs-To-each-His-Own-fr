// ћэри  аспер - взаимна€ любовь и офицер
void ProcessDialogEvent()
{
	...
	// ---------------------------------покушение на јкулу - вариант R --------------------------------
		// разговор в каюте јдольфа
		case "Cabin":
	...
	// ---------------------------------покушение на јкулу - вариант M --------------------------------
		...
		case "happy_9":
			...
			pchar.questTemp.LSC.MaryBye = "true"; // атрибут об€зательного прощани€ перед телепортацией
			LocatorReloadEnterDisable("LostShipsCity_town", "reload62", false);
			LocatorReloadEnterDisable("LostShipsCity_town", "reload63", false); // открываем ÷ерес снаружи
		break;
	...
	// ----------------------------------вместе с ћэри на ÷ерес —мити------------------------------------------
		case "LSC_love":
			...
			if (CheckAttribute(pchar, "questTemp.LSC.FindDolly")) // нашел статую - прощаетс€
			{
	...
	// --> взаимоотношени€ - секс и прочее
		...
		case "dolly_16":
			DialogExit();
			DeleteAttribute(pchar, "questTemp.LSC.MaryBye"); // можно телепортироватьс€
		...
		case "narval_12":
			...
			// владение Ќарвалом повышает скилл “ќ у ћэри и открывает перк т€желой руки
	...
	// --------------------------------------вернулс€ на ќстров---------------------------------------------
		...
		case "adversary_5":
			...
			// проход первый - √√ либо соглашаетс€, либо нет, отказ ведет к полному разрыву
		...
		case "adversary_fail_4": // обидел ћэри - больше от нее ничего не получишь
			DialogExit();
			LAi_CharacterDisableDialog(npchar);//запрет диалога
			for (i=0; i<=3; i++)
			{
				LocatorReloadEnterDisable("LostShipsCity_town", "reload6"+i, true); // закрываем вход к ћэри
			}
		...
		// соглашаемс€
		case "adversary_hire":
			// проход второй - смотрим, Ёлен может стать подругой или нет
		...
		case "adversary_hire_no": // ћэри не пойдет, пока не √√ не расстанетс€ с Ёлен. ¬ыбирай, кто тебе милее...
		...
		case "adversary_hire_no_7":
			...
			WaitDate("", 0, 0, 0, 1, 10); //крутим врем€
			RecalculateJumpTable();
			NextDiag.CurrentNode = "LSC_love"; // оставл€ем штучки с ћэри на ќстрове в силе
		...
		// повторный разговор после расставани€ с Ёлен
		...
		case "adversary_hire_yes": // Ёлен заведомо не друг - ћэри идет сразу, либо если выбрал ћэри
			dialog.text = "Really? "+pchar.name+"! I will always follow you! I will never lie to you, I will never betray you, I swear!";
			link.l1 = "I believe you, my dear... Neither will I."; // а куда же ты денешьс€ )
	...
	// --> консультации по морским сражени€м
		...
		case "sea_bomb_5":
			...
			AddQuestUserData("BarbTemptation", "sName", "ћэри");
		...
		case "on_coast_4":
			dialog.text = "That's for sure... Was it the 'Flying Dutchman', cap? Why did they attack us? And why didnТt they shoot at us before the boarding, but sank the ship with just one salvo later?";
	...
	// --> ћэри вне LSC
		// секс - ћэри не отказывает никогда
		case "cabin_sex":
		...
		// ругаетс€ за то, что ходил по бордел€м, √√ изворачиваетс€
		case "brothel":
		...
		case "brothel_8":
			...
			SetLaunchFrameFormPic("loading\inside\kiss.tga");
			LaunchFrameForm();
			pchar.GenQuest.BrothelLock = true; // все, больше не походишь по бордел€м :)
		...
		case "GoldenGirl_4":
			dialog.text = "Whew, donТt bother. Just tell me straight that I am not good enough for those pricks in cuffs. A simple girl for fun, huh? Ashamed to show yourself with her in the world? Not good enough for fancy reception, right, honorable monsieur Charles de Maure?";
		...
		case "GoldenGirl_18":
			...
			link.l1 = "I have to. It's complicated, but in short: I won his ship and hisЕ dame. He publicly accused me of cheating and challenged me for a duel. This man is a bastard of a very influential nobleman, Count de Levi Vantadur. I will have to do my best to don't kill him.";
			link.l1.go = "GoldenGirl_19";
		break;
		
		case "GoldenGirl_19":
			dialog.text = "A dame? I can understand playing for a ship, butЕ a woman, yes? They were saying the truth in the streets alright! You admit it just like that?! Like I amЕ one of your fucking sailors?";
		...
		case "GoldenGirl_20":
			dialog.text = "Well, maybe it is a casual thing to do among you nobles but I am not like that, yes. Do what you like, say what you likeЕ go and die for her if you want! I will go to the fucking ship you won and get fucking wasted. I so believed this was for real and... will last forever, I hoped so much!";
		...
		case "GoldenGirl_26":
			dialog.text = "Already did. I am sorry too, alright! If only I didn't took your dealings with that dame too personalЕ I would had set a proper watch that night. They would never took us down so easily! So silly of me!";
		...
		case "GoldenGirl_36_1":
			...
			link.l1 = "I have no idea, Mary. We are together again, the rest is irrelevant. If those in power will fail to sweep it under the rugЕ well, I am ready to do my duty before the Crown.";
		...
		case "LongHappy_8":
			...
			else // —ага провалена по времени или утоплен ÷ентурион
			{
				dialog.text = "You know, I donТt have many friends except our guys from the crew. So invite those whom you consider necessary, and I will just be happy, alright!";
		...
		case "LongHappy_19":
			DialogExit();
			pchar.GenQuest.CannotWait = true;//запрет ожидани€
		...
		case "LongHappy_32":
			if (CheckAttribute(pchar, "questTemp.LongHappy.MarrySP")) dialog.text = "Thank you, Charles, thank you... Everything is so solemn and wonderful! This is the best day of my life, alright!!! Look, how happy is everyone around! But, it's time for you to walk around the guests and accept congratulations!  ак закончишь - возвращайс€ - загл€нем к реб€там в таверну.";
		...
		case "LongHappy_47":
			dialog.text = "He is wounded, but he will be okay. You canТt easily break our baron, alright! He's just woken up and is already eager for battle.";
		...
		case "LongHappy_49":
			...
			else
			{
				...
				{
					dialog.text = "I feel so bad about Gino, Charles, alright! When I found out, I didnТt believe it! How are you?";
		...
		case "LongHappy_50":
			dialog.text = "You barely stand on your feet, alright! Come on, Dannie found us a house here on the outskirts, I donТt want to return to the ship now. We'll sit and drink to our fallen friends. This all is too much for sober head, alright.";
		...
		case "LongHappy_53":
			...
			link.l1 = "Jan gave me some kind of tip... It seems to me that our harsh maskogue went on the razzle-dazzle. LetТs take a walk to Sabo-Matila Cove, a few people saw him leaving in that direction.";
		...
		case "LongHappy_59":
			...
			link.l1 = "IТve been yours for a long time already, and will be forever, my love...";
		...
		//--> ----------------------------------- офицерский блок ------------------------------------------
		case "Mary_officer":
			// если шл€лс€ по бордел€м - устроит небольшой скандал 
